...
async function getsongs(folder) {
    ...
    songdiv.innerHTML = `<div class="search ">

    <div class="searchsvg">
        <img src="svg/search.svg" alt="search" width="100">
    </div>
...
async function getFolder() {
    ...
    Array.from(as).forEach(element => {
        if (element.href.includes("/songs/")) {
            ...
            <div class="cover">
                <img src="images/co.jpg" alt="cover image" width="100px">
            </div>
            <div class="info">
                <h2>${fname}</h2>
                <p> </p>
...
async function main() {
    ...
    function removeCardStyle() {
        let ind = list.indexOf(song.currentSrc.split(`/${currentfolder}/`)[1]);
        let cards = document.querySelectorAll(".songs .card");
    ...
    song.addEventListener("playing", () => {
        ...
        let name = song.currentSrc.split(`/${currentfolder}/`)[1].replaceAll("%20", " ").split("_")[0];
        let artist = song.currentSrc.split(`/${currentfolder}/`)[1].replaceAll("%20", " ").split("_")[1]
